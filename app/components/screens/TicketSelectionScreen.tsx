...
export default function TicketSelectionScreen({ onNavigate, selectedEvent }: TicketSelectionScreenProps) {
  ...
  // Usar el evento seleccionado o valores por defecto
  ...
  return (
    <MobileContainer>
      ...
      <div className="h-full overflow-y-auto pb-24">
        <div className="p-4">
          {/* Event Info */}
          <div className={`bg-gradient-to-r ${event.gradient} rounded-2xl p-4 mb-6 text-white`}>
            <h2 className="font-bold text-2xl mb-1">{event.artist} - {event.tour}</h2>
            <p className="opacity-90">{event.date}</p>
            <p className="opacity-90">{event.venue}, {event.city}</p>
          </div>
